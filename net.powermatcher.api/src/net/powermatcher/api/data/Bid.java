...
/**
 * This immutable abstract data object defines the basis for a Bid in the
 * Powermatcher cluster. The bid curve can be represented in several ways, so
 * subclasses will have their own implementation.
 * 
 * @author FAN
 * @version 2.0
 */
public abstract class Bid {
	public static Bid flatDemand(MarketBasis marketBasis, int bidNumber,
			double demand) {
		return new PointBid.Builder(marketBasis).setBidNumber(bidNumber)
				.add(0, demand).build();
	}
	...
	protected Bid(MarketBasis marketBasis, int bidNumber) {
		if (marketBasis == null) {
			throw new IllegalArgumentException(
					"marketBasis is not allowed to be null");
		}
	...
	/**
	 * Aggregates this {@link Bid} instance with another Bid. Bid are aggregated
	 * by adding their bid curves.
	 * 
	 * @param other
	 *            The {@link Bid} whose bid curve has to be added to the bid
	 *            curve of this instance.
	 * @return A new aggregated Bid.
	 */
	public abstract Bid aggregate(final Bid other);

	/**
	 * Calculates the {@link Price} at the intersection with the bid curve at
	 * the given demand.
	 * 
	 * @param targetDemand
	 *            the part of the bid curve you want to get the {@link Price}
	 *            of.
	...
	/**
	 * Calculates the demand at the intersection in the bid curve with the
	 * priceStep in a demand array.
	 * 
	 * @param price
