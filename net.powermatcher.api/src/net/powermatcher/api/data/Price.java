...
public class Price implements Comparable<Price> {
	...
	public Price(MarketBasis marketBasis, double price) {
		if (marketBasis == null) {
			throw new IllegalArgumentException(
					"marketBasis not allowed to be null");
		} else if (Double.isNaN(price)) {
			throw new IllegalArgumentException("Price NaN is not valid");
		} else if (price < marketBasis.getMinimumPrice()
				|| price > marketBasis.getMaximumPrice()) {
			throw new IllegalArgumentException("Price " + price
					+ " is out of bounds [" + marketBasis.getMinimumPrice()
	...
	public PriceStep toPriceStep() {
		double priceStep = (priceValue - marketBasis.getMinimumPrice())
	...
	@Override
	public int hashCode() {
		return 83257 * marketBasis.hashCode() + 50723
				* Double.valueOf(priceValue).hashCode();
	}
	...
	/**
	 * The {@link DecimalFormat} used to format the <code>String</code>
	 * representation of this instance's priceValue.
	 */
	 ...
	 * 
	 * This method compares the priceValues of both instances. The one with the
	 * highest value is the greatest.
	 * 
	 * @param that
