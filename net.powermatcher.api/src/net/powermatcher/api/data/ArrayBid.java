...
/**
 * This immutable data object represents a {@link Bid} with a
 * <code>double</code> demand array to represent the bid curve. This is used
 * because it is easy to perform calculations with.
 * 
 * @author FAN
 * @version 2.0
 */
public class ArrayBid extends Bid {
	...
	/**
	 * In the calculatePricePoints, 2 floating points will be compared. The
	 * significance is set here.
	 */
	...
	public static final class Builder {
		 ...
		 * 
		 * @param demand
		 *            The demand to be added to the demand array. Must not be
		 *            higher than the previous added demand.
		 * @return this instance of the Builder, with the added demand
		 * @throws IllegalArgumentException
		 *             when the demand to be added is higher than the demand
		...
		public Builder setDemand(double demand) {
			...
			if (nextIndex > 0 && demand > builderDemand[nextIndex - 1]) {
				throw new IllegalArgumentException(
						"The demand can not be ascending");
			}
		...
		/**
		 * {@link Checksum} to see if the given index is not bigger than the
		 * length of builderDemand.
		 * 
		 * @param ix
		 *            the index that has to be checked
		 * @throws ArrayIndexOutOfBoundsException
		 *             if the given index is bigger than the length of
		 *             builderDemand.
		 */
		private void checkIndex(int ix) {
			if (ix >= builderDemand.length) {
				throw new ArrayIndexOutOfBoundsException(
						"Demand array has already been filled to maximum");
			}
		}

		/**
		 * Sets the demandArray with the supplied demand array. The supplied
		 * array must not be ascending. The length of the array should be the
		 * same size as the number of price steps in the marketBasis.
		 * 
		 * @param demand
		 *            The new demand array
		 * @return this instance of the Builder, with the added array
		 * @throws IllegalArgumentException
		 *             if the size of the array differs from the number of
		 *             priceSteps in the MarketBasis or if the demand array is
		 *             ascending
		 */
		...
		/**
		 * Makes sure the whole array is filled, then creates the ArrayBid with
		 * the Builder's internal values
		 * 
		 * @return The created ArrayBid
		 * @throws IllegalArgumentException
		 *             When the length of the demandArray is not equal to the
		...
		/**
		 * fills the demand array from the nextIndex until the designated
		 * priceStep with the last set demand Will do nothing on an already
		 * filled array.
		 * 
		 * @param priceStep
		 ...
		 * @throws IllegalArgumentException
		 *             when the supplied priceStep is higher than the number of
		 *             price steps in the marketBasis
		 */
		public Builder fillArrayToPriceStep(int priceStep) {
			if (nextIndex == 0) {
				throw new IllegalStateException(
						"Demand array contains no demand that can be extended");
			}
			if (priceStep > marketBasis.getPriceSteps()) {
				throw new IllegalArgumentException(
						"The supplied priceStep is out of bounds");
			}
	...
	public ArrayBid(MarketBasis marketBasis, int bidNumber, double[] demandArray) {
		...
		if (demandArray.length != marketBasis.getPriceSteps()) {
			throw new IllegalArgumentException(
					"Length of the demandArray is not equal to the number of price steps");
		}
	...
	/**
	 * A copy constructor to create a copy of the given ArrayBid and its
	 * bidNumber.
	 * 
	 * @param bid
	...
	public ArrayBid(ArrayBid bid, int bidNumber) {
		...
		this.demandArray = Arrays.copyOf(bid.demandArray,
				bid.demandArray.length);
	}
	...
	/**
	 * A constructor used to create an instance of this class, based on a
	 * {@link PointBid}.
	 * 
	 * @param base
	...
	public ArrayBid aggregate(Bid other) {
		if (!other.marketBasis.equals(marketBasis)) {
			throw new IllegalArgumentException(
					"The marketbasis of the supplied bid does not equal this market basis");
		}
	...
	private int determineDropoff(final double targetDemand, int rightBound) {
		while (rightBound < demandArray.length
				&& demandArray[rightBound] >= targetDemand) {
			rightBound += 1;
		}
		return rightBound;
	}
	...
	/**
	 * The index of the point which is just above the intersection is now stored
	 * in the variable 'middle'. This means that middle + 1 is the index of the
	 * point that lies just under the intersection. That means that the exact
	 * intersection is between middle and middle+1, hence a weighted
	 ...
	 */
	private int interpolate(final double targetDemand, int leftBound,
			int rightBound) {
		int priceStep;
	...
	/**
	 * Pricing for boundary cases: 1) If the curve is a flat line, if the demand
	 * is positive the price will become the maximum price, or if the demand is
	 * zero or negative, the price will become 0. 2) If the curve is a single
	 ...
	 * @return
	 */
	private int determineCurve(final double targetDemand, int leftBound,
			int rightBound) {
		int priceStep;
		if (leftBound == 0 && rightBound == demandArray.length) {
			...
			if (demandArray[leftBound] != 0) {
				...
				priceStep = new Price(marketBasis, 0).toPriceStep()
						.getPriceStep();
			}
		} else {
			/* Curve is a single step at leftBound */
			priceStep = demandArray[leftBound] <= targetDemand ? leftBound
					: leftBound + 1;
		}
		return priceStep;
	}
	...
	public double getDemandAt(final PriceStep priceStep) {
		if (!priceStep.getMarketBasis().equals(marketBasis)) {
			throw new IllegalArgumentException(
					"The marketbasis of the pricestep does not equal this market basis");
		}
	...
	/**
	 * @return a {@link PricePoint} <code>array</code> representation of the
	 *         demand array.
	 */
	PricePoint[] calculatePricePoints() {
		...
		/*
		 * Flag to indicate if the last price point is the end point of a flat
		 * segment.
		 */
		...
		while (i < priceSteps - 1) {
			/*
			 * Search for the last price step in the flat segment, if any. At
			 * the end of this loop delta is the difference for the next demand
			 * after the flat segment.
			 */
			PricePoint flatEndPoint = null;
			double delta = 0.0d;
			while (i < priceSteps - 1
					&& (delta = this.demandArray[i] - this.demandArray[i + 1]) == 0) {
				i += 1;
				flatEnd = true;
			}
			/*
			 * Add a point at i for the following two cases: 1) If not at the
			 * end of the demand array, add the next point for i and remember
			 * that this point is the end of a flat segment. 2) Add a final
			 * point if past the end of the demand array, but the previous point
			 * was not the beginning of a flat segment.
			 */
			 ...
			 */
			if (i < priceSteps - 1) {
				if (Math.abs((this.demandArray[i] - this.demandArray[i + 1])
						- delta) < Math.pow(10, PRECISION * -1)) {
					/*
					 * Here i is in a constantly inclining or declining segment.
					 * Search for the last price step in the segment.
					 */
					while (i < priceSteps - 1
							&& Math.abs((this.demandArray[i] - this.demandArray[i + 1])
									- delta) < Math.pow(10, PRECISION * -1)) {
						i += 1;
					}
					/*
					 * If not at the end of the demand array, add the end point
					 * for the segment.
					 */
					if (i < priceSteps - 1) {
					 ...
					 * segment, and the previous segment was flat, then move the
					 * end point of the flat segment one price step forward to
					 * convert it to a straight step.
					 * 
					 * This is to preserve the semantics of the straight step
					 * when converting between point and vector representation
					 * and back.
					 */
					flatEndPoint = newPoint(i);
				}
	...
	/**
	 * Adds a point at location for the following two cases: 1) Add a point for
	 * the start of the next flat segment and loop. 2) Add a final point if the
	 * last step of the demand array is the end of an inclining or declining
	 * segment.
	 * 
	 * @param priceSteps
	 * @param points
	 * @param flatStart
	 * @param location
	 * @return boolean indicating if the nextPoint is the start of a new flat
	 *         segment
	 */
	private boolean endSegment(int priceSteps, List<PricePoint> points,
			boolean flatStart, int location) {
		if (location == priceSteps - 1
				|| (location < priceSteps - 1 && this.demandArray[location]
						- this.demandArray[location + 1] == 0)) {
	 ...
	 * 
	 * @param priceStep
	 *            the pricestep used to calculate the {@link PriceStep} and
	 *            determin the demand that belongs to that value
	 * @return a {@link PricePoint}, based on on the priceStep paramater.
	 */
	private PricePoint newPoint(int priceStep) {
		return new PricePoint(new PriceStep(marketBasis, priceStep).toPrice(),
				this.demandArray[priceStep]);
	}
	...
	@Override
	public int hashCode() {
		return 2011 * Arrays.hashCode(demandArray) + 3557 * bidNumber
