...
/**
 * Validate the correct behaviour of the components when incorrect prices are
 * inserted.
 * 
 * @author FAN
 * @version 2.0
...
public class SendReceivePriceTestCPQ1 extends BidResilienceTest {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(SendReceivePriceTestCPQ1.class);

	@Rule
	public ExpectedException exception = ExpectedException.none();

	/**
	 * The auctioneer is invoked to publish a null price. The auctioneer will
	 * not publish the null price but reset its publish-state.
	 * 
	 * Check if the value of the last published price is not the null price but
	 * the most recent valid price.
	 * 
	 * @throws IOException
	 * @throws DataFormatException
	 */
	@Test
	public void publicationNullPriceAuctioneerCPQ1() throws IOException,
			DataFormatException {
		...
		assertEquals(this.resultsReader.getEquilibriumPrice(),
				this.concentrator.getLastPrice().getPrice().getPriceValue(), 0);
		...
		assertEquals(true,
				(this.concentrator.getLastReceivedPriceUpdate() == null));
	 ...
	 * concentrator is sent directly a null price. The concentrator should
	 * reject it.
	 * 
	 * @throws IOException
	 * @throws DataFormatException
	...
	public void rejectReceivalNullPriceConcentratorCPQ1() throws IOException,
			DataFormatException {
		...
		assertEquals(this.resultsReader.getEquilibriumPrice(),
				this.concentrator.getLastReceivedPriceUpdate().getPrice()
						.getPriceValue(), 0);
		...
		assertEquals(this.resultsReader.getEquilibriumPrice(),
				this.concentrator.getLastPublishedPriceUpdate().getPrice()
						.getPriceValue(), 0);
	}
	 ...
	 * an price that is outside its local price range. According to the
	 * specifications this is permitted. Connected agents can have a different
	 * local price market base.
	 * 
	 ...
	 */
	@Test
	public void sendPriceOutsideMarketBaseAuctioneerCPQ1() throws IOException,
			DataFormatException {
		...
		// Now uses concentrator.getLastReceived as
		// auctioneer.getLastPublishedPrice is not reliable anymore
		assertEquals(priceUpdate.getPrice().getPriceValue(), this.concentrator
				.getLastReceivedPriceUpdate().getPrice().getPriceValue(), 0);
	 ...
	 */
	@Test
	public void rejectPriceOutsideRangeConcentratorCPQ1() throws IOException,
			DataFormatException {
		...
		// Validate if concentrator has received the new price
		assertEquals(price.getPriceValue(), this.concentrator.getLastPrice()
	 ...
	 * an price that is outside its local price range. According to the
	 * specifications this is permitted. Connected agents can have a different
	 * local price market base. Check if the concentrator publishes this price.
	 * 
	 * 
	 * 
	 * @throws IOException
	 ...
	 */
	@Test
	public void publishPriceOutsideRangeConcentratorCPQ1() throws IOException,
			DataFormatException {
		...
		assertEquals(price.getPriceValue(), this.concentrator
				.getLastPublishedPriceUpdate().getPrice().getPriceValue(), 0);
	}
	 ...
	 * local price market base.
	 * 
	 * Check if the concentrator accepts the price and forwards the price to the
	 * agents.
	 * 
	 * @throws IOException
	 * @throws DataFormatException
	 */
	@Test
	public void retainmentLastValidPriceConcentratorCPQ1() throws IOException,
			DataFormatException {
	 ...
	 * A set of agents send a bid to the auctioneer via the concentrator. After
	 * sending a calculated (valid) price the auctioneer will be forced to send
	 * an price that is outside its local price range. Connected agents can have
	 * a different local price market base.
	 * 
	 ...
	 */
	@Test
	public void rejectPriceOutsideRangeByAgentsCPQ1() throws IOException,
			DataFormatException {
		...
		for (MockAgent agent : agentList) {
			assertEquals(price.getPriceValue(), agent.getLastPriceUpdate()
					.getPrice().getPriceValue(), 0);
		}
	}
}
