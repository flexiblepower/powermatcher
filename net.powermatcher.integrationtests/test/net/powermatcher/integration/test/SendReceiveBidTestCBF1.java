...
public class SendReceiveBidTestCBF1 extends BidResilienceTest {
	 ...
	 */
	@Test
	public void sendBidToUpStreamComponentCBF1() throws IOException,
			DataFormatException {
		...
		do {
			...
			if (bid != null) {
				...
				// auctioneer
				ArrayBid lastReceivedBid = this.auctioneer.getLastReceivedBid()
						.toArrayBid();
				ArrayBid aggregatedBid = auctioneer.getAggregatedBid(
						marketBasis).toArrayBid();
				assertThat(lastReceivedBid.getDemand(),
						is(equalTo(aggregatedBid.getDemand())));
				assertThat(lastReceivedBid.getMarketBasis(),
						is(equalTo(aggregatedBid.getMarketBasis())));
				...
				PriceUpdate lastPriceUpdate = this.agentList.get(id)
						.getLastPriceUpdate();
				PriceUpdate lastReceivedPriceUpdate = this.concentrator
						.getLastReceivedPriceUpdate();
				assertThat(lastPriceUpdate.getPrice(),
						is(equalTo(lastReceivedPriceUpdate.getPrice())));
			} else {
				stop = true;
			}
