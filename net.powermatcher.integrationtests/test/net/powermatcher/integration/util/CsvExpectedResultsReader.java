...
public class CsvExpectedResultsReader {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(CsvExpectedResultsReader.class);
	...
	public CsvExpectedResultsReader(String filename) throws IOException,
			DataFormatException {
		super();
	...
	private void readResultsData() throws IOException, DataFormatException {
		...
		this.aggregatedBid = new ArrayBid(marketBasis, 0,
				createAggregatedBid(demandList));
	...
	private double[] createAggregatedBid(List<String> demandList)
			...
			for (String item : demandList) {
				try {
					...
					throw new DataFormatException(
							"Exception while parsing demand string : cannot convert"
									+ item + " to double");
				}
				i++;
			}
	...
	private double createEquilibriumPrice(List<String> priceList)
			throws DataFormatException {
		if (priceList != null && priceList.size() == 1) {
			try {
				...
				throw new DataFormatException(
						"Parse exception : cannot convert equilibrium"
								+ priceList.get(0) + " to double");
			}
		} else {
	...
	private MarketBasis createMarketBasis(List<String> marketBasisData)
			throws DataFormatException {
		if (marketBasisData != null && !marketBasisData.isEmpty()) {
			...
			try {
				...
				MarketBasis mb = new MarketBasis(commodity, currency,
						priceSteps, minimumPrice, maximumPrice);
				this.marketBasis = mb;
			} catch (NumberFormatException e) {
