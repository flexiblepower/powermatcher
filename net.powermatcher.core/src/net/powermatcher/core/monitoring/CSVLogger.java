...
/**
 * {@link CSVLogger} is an implementation of {@link AgentEventLogger} where the
 * {@link AgentEvent}s are logged to a comma separated file.
 * 
 * @author FAN
 * @version 2.0
 */
...
public class CSVLogger extends AgentEventLogger {
	...
	private static final String[] BID_HEADER_ROW = new String[] { "logTime",
			"clusterId", "agentId", "qualifier", "commodity", "currency",
			"minimumPrice", "maximumPrice", "minimumDemand", "maximumDemand",
			"effectiveDemand", "effectivePrice", "lastUpdateTime", "bidNumber",
			"demand", "pricePoints" };
	...
	private static final String[] PRICE_HEADER_ROW = new String[] { "logTime",
			"clusterId", "id", "qualifier", "commodity", "currency",
			"minimumPrice", "maximumPrice", "priceValue", "lastUpdateTime" };

	private static final String[] WHITELIST_HEADER_ROW = new String[] {
			"logTime", "clusterId", "agentId", "blockedAgentId",
			"lastUpdateTime" };

	private static final String[] PEAKSHAVING_HEADER_ROW = new String[] {
			"logTime", "clusterId", "agentId", "floor", "ceiling", "oldDemand",
			"newDemand", "oldPrice", "newPrice" };

	/**
	 * OSGI configuration of the {@link CSVLogger}
	 */
	public static interface Config {
		@Meta.AD(required = false, description = "Filter for specific agentId's. When no filters are supplied, it will log everything.")
		List<String> filter();

		@Meta.AD(name = "eventType", description = "The AgentEventType this logger has to log.")
		AgentEventType eventType();

		@Meta.AD(deflt = "event_log_::yyyyMMdd::.csv", description = "The pattern for the file name of the log file. Dataformat strings are placed between the delimeter '::'")
	...
	/**
	 * The filter containing the {@link ObservableAgent}s that have to be
	 * monitored
	 */
	...
	public void addObservable(ObservableAgent observable,
	...
	protected void processConfig(Map<String, Object> properties) {
		...
		this.logFile = createLogFile(config.logFilenamePattern(),
				config.logLocation());
		if (!logFile.exists()) {
			String[] header = null;
	...
	/**
	 * Creates a new {@link File} to write the csv lines to. It also parses
	 * possible {@link DateFormat} strings in the fileName parameter.
	 * 
	 * @param fileName
	 *            the name of the {@link File} that has to be created
	 * @param logLocation
	 *            the location of the {@link File} that has to be created
	 * @return The {@link File} with the fileName name and the logLocation as
	 *         location
	 */
	private File createLogFile(String fileName, String logLocation) {
		...
		if (fileName.matches("\\S*::\\w*::*.csv")) {
			...
			String logDateFormat = fileName.substring(
					fileName.indexOf("::") + 2, fileName.lastIndexOf("::"));
			String date = new SimpleDateFormat(logDateFormat)
					.format(timeService.currentDate());
	 ...
	 * 
	 * @param line
	 *            the comma separated line that has to be written to the
	 *            outputFile
	 * @param outputFile
	 ...
	 * 
	 * @param scheduler
	 *            The {@link ScheduledExecutorService} implementation to be
	 *            injected
	 */
	@Reference
	...
	protected void dumpLogs() {
		for (LogRecord logRecord : getLogRecords().toArray(
				new LogRecord[getLogRecords().size()])) {
	...
	/**
	 * Creates a <code>String[]</code> out of a {@link PeakShavingLogRecord} to
	 * be used in {@link CSVLogger#writeLineToCSV(String[], File)}
	 * 
	 * @param logRecord
	 *            the {@link PeakShavingLogRecord} that has to be transformed
	 * @return the <code>String[]</code> representation of logRecord
	 */
	private String[] createLineForPeakShavingLogRecor(
			PeakShavingLogRecord logRecord) {
		...
		String oldPrice = logRecord.getOldPrice() == null ? "" : String
				.valueOf(logRecord.getOldPrice());
		String newPrice = logRecord.getNewPrice() == null ? "" : String
				.valueOf(logRecord.getNewPrice());

		return new String[] { getDateFormat().format(logRecord.getLogTime()),
				logRecord.getClusterId(), logRecord.getAgentId(),
				String.valueOf(logRecord.getFloor()),
				String.valueOf(logRecord.getCeiling()),
				oldDemandBuilder.toString(), newDemandBuilder.toString(),
				oldPrice, newPrice };
	}

	/**
	 * Creates a <code>String[]</code> out of a {@link WhitelistLogRecord} to be
	 * used in {@link CSVLogger#writeLineToCSV(String[], File)}
	 * 
	 * @param logRecord
	 *            the {@link WhitelistLogRecord} that has to be transformed
	 * @return the <code>String[]</code> representation of logRecord
	 */
	private String[] createLineForWhitelistLogRecord(
			WhitelistLogRecord logRecord) {
		return new String[] { getDateFormat().format(logRecord.getLogTime()),
				logRecord.getClusterId(), logRecord.getAgentId(),
				logRecord.getBlockedAgent(),
	...
	/**
	 * Creates a <code>String[]</code> out of a {@link BidLogRecord} to be used
	 * in {@link CSVLogger#writeLineToCSV(String[], File)}
	 * 
	 * @param logRecord
		...
		if (bid instanceof ArrayBid) {
			...
			if (temp.getPricePoints() != null) {
				...
				for (PricePoint p : temp.getPricePoints()) {
					...
					pricePointBuiler.append(MarketBasis.PRICE_FORMAT.format(p
							.getPrice().getPriceValue()));
					pricePointBuiler.append("|").append(
							MarketBasis.DEMAND_FORMAT.format(p.getDemand()));
				}
			}
		}

		return new String[] {
				getDateFormat().format(logRecord.getLogTime()),
				logRecord.getClusterId(),
				logRecord.getAgentId(),
				logRecord.getQualifier().getDescription(),
				...
				getDateFormat().format(logRecord.getEventTimestamp()),
				String.valueOf(bid.getBidNumber()), demandBuilder.toString(),
				pricePointBuiler.toString() };
	}

	/**
	 * Creates a <code>String[]</code> out of a {@link PriceUpdateLogRecord} to
	 * be used in {@link CSVLogger#writeLineToCSV(String[], File)}
	 * 
	 * @param logRecord
	...
	private String[] createLineForPriceUpdateLog(PriceUpdateLogRecord logRecord) {
		MarketBasis marketbasis = logRecord.getPriceUpdate().getPrice()
				.getMarketBasis();
		...
		return new String[] {
				...
				MarketBasis.PRICE_FORMAT.format(marketbasis.getMinimumPrice()),
				MarketBasis.PRICE_FORMAT.format(marketbasis.getMaximumPrice()),
				MarketBasis.PRICE_FORMAT.format(logRecord.getPriceUpdate()
						.getPrice().getPriceValue()),
				getDateFormat().format(logRecord.getEventTimestamp()) };
	}
}
