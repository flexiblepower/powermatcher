...
/**
 * <p>
 * This class represents a {@link PeakShavingConcentrator} component where
 * several instances can be created.
 * </p>
 * 
 * <p>
 * The {@link PeakShavingConcentrator} receives {@link Bid} from the agents and
 * forwards this in an aggregate {@link Bid} up in the hierarchy to a
 * {@link PeakShavingConcentrator} or to the {@link Auctioneer}. It will receive
 * price updates from the {@link Auctioneer} and forward them to its connected
 * agents. TODO: add PeakShavingConcentrator comment.
 * 
 * @author FAN
 * @version 2.0
 */
...
public class PeakShavingConcentrator extends BaseAgent implements
	...
	public synchronized boolean connectToAgent(Session session) {
		...
		this.aggregatedBids.updateBid(session.getAgentId(),
				new ArrayBid.Builder(this.sessionToMatcher.getMarketBasis())
						.setDemand(0).build());
	...
	public void updateBid(Session session, Bid newBid) {
		...
		this.publishEvent(new IncomingBidEvent(session.getClusterId(), config
				.agentId(), session.getSessionId(), timeService.currentDate(),
				"agentId", newBid, Qualifier.AGENT));
	...
	protected synchronized void doBidUpdate() {
		if (sessionToMatcher != null) {
			ArrayBid aggregatedBid = this.aggregatedBids.getAggregatedBid(
					this.sessionToMatcher.getMarketBasis(), true);
			...
			publishEvent(new PeakShavingEvent(config.agentId(),
					this.getClusterId(), timeService.currentDate(), this.floor,
					this.ceiling, aggregatedBid.getDemand(),
	...
	public void updatePrice(PriceUpdate priceUpdate) {
		...
		this.publishEvent(new IncomingPriceUpdateEvent(sessionToMatcher
				.getClusterId(), this.config.agentId(), this.sessionToMatcher
				.getSessionId(), timeService.currentDate(), priceUpdate,
				Qualifier.AGENT));

		// Find bidCacheSnapshot belonging to the newly received price update
		BidCacheSnapshot bidCacheSnapshot = this.aggregatedBids
	 ...
	 *            The {@link PriceUpdate} whose {@link Price} has to be
	 *            adjusted.
	...
	private synchronized PriceUpdate adjustPrice(
		...
		if (!Double.isNaN(this.ceiling) && allocation > this.ceiling) {
			priceOutIndex = this.findFirstIndexOfUnclippedRegion(
					demandFunction.getDemand(), this.ceiling);
			...
			priceOutIndex = this.findLastIndexOfUnclippedRegion(
					demandFunction.getDemand(), this.floor);
		...
		// set the new price
		this.priceIn = newPriceUpdate.getPrice();
		this.priceOut = new Price(newPriceUpdate.getPrice().getMarketBasis(),
	...
	private synchronized ArrayBid transformAggregatedBid(
		...
		if (!Double.isNaN(this.getUncontrolledFlow())) {
			newAggregatedBidOut = newAggregatedBidOut.transpose(this
					.getUncontrolledFlow());
		}
	...
	/**
	 * @return the difference between the measured flow and the allocation for
	 *         the cluster.
	 */
	...
	/**
	 * @return the demand at the value of the priceOut in the bid curve of the
	 *         aggregated {@link Bid}.
	 */
