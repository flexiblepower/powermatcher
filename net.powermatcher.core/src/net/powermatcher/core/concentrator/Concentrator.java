 ...
 * @version 2.0
 */
@Component(designateFactory = Concentrator.Config.class, immediate = true, provide = {
		ObservableAgent.class, MatcherEndpoint.class, AgentEndpoint.class,
		WhitelistableMatcherEndpoint.class })
public class Concentrator extends BaseAgent implements MatcherEndpoint,
	...
	/**
	 * The scheduler that is used to schedule the bid updates during activation
	 * and thus must be set before activation.
	 */
	private ScheduledExecutorService scheduler;

	/**
	 * The schedule that is running the bid updates. This is created in the
	 * {@link #activate(Map)} method and cancelled in the {@link #deactivate()}
	 * method.
	 */
	...
	public synchronized boolean connectToAgent(Session session) {
		...
		if (validAgents.size() == 0
			...
			this.aggregatedBids
					.updateBid(session.getAgentId(), new ArrayBid.Builder(
			...
			LOGGER.warn("Agent [{}] is not on whitelist, reject connection",
					session.getAgentId());
			publishEvent(new WhitelistEvent(this.getAgentId(),
					session.getAgentId(), this.getClusterId(),
	...
	/**
	 * This method should be overridden when the bid that will be sent has to be
	 * changed.
	 * 
	 * @param aggregatedBid
	 *            The (input) aggregated bid as calculated normally (the sum of
	 *            all the bids of the agents).
	 * @return The bid that will be sent to the matcher that is connected to
	 *         this {@link Concentrator}.
	 */
	...
	public boolean equals(Object obj) {
		...
		return this.aggregatedBids.equals(other.aggregatedBids)
				...
				&& this.sessionToAgents.equals(other.sessionToAgents)
	...
	public int hashCode() {
		return super.hashCode()
				...
				* ((this.aggregatedBids == null ? 0 : aggregatedBids.hashCode())
						+ (this.servicePid == null ? 0 : servicePid.hashCode())
						+ this.sessionToMatcher.hashCode()
