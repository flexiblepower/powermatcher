...
/**
 * <p>
 * This class represents an {@link ObjectiveAuctioneer} component which will
 * receive all {@link Bid} of other agents as a single {@link Bid} or as an
 * aggregate {@link Bid} via one or more {@link Concentrator}. If
 * {@link ObjectiveAgent} are active, the {@link ObjectiveAuctioneer} will also
 * receive a {@link Bid} from the {@link ObjectiveAgent} as a single {@link Bid}
 * .
 * </p>
 * 
 * <p>
 ...
 * agents in the topology and the objective agent. This equilibrium is
 * communicated to the agents down the hierarchy in the form of price update
 * messages and to the objective agent.
 * 
 * In order of aggregation the {@link Bid} from the device agents and objective
 * agents, the {@link ObjectiveAuctioneer} will first aggregate the device
 * agents bid and secondly aggregate the {@link Bid} from the objective agent.
 * After the aggregation the {@link ObjectiveAuctioneer} will determine the
 * price and sends it to the {@link Concentrator} / device agents and the
 * objective agent.
 * </p>
 * 
 * @author FAN
 * @version 2.0
 */
@Component(designateFactory = ObjectiveAuctioneer.Config.class, immediate = true, provide = {
		ObservableAgent.class, MatcherEndpoint.class })
public class ObjectiveAuctioneer extends Auctioneer {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(ObjectiveAuctioneer.class);
	...
	public void addObjectiveEndpoint(ObjectiveEndpoint objectiveEndpoint) {
		...
		if (agent.getAgentId() != null) {
			...
			throw new IllegalStateException(
					"Only one objective agent can be added to the cluster");
		}
	}
	...
	public synchronized boolean connectToAgent(Session session) {
		...
		this.aggregatedBids.updateBid(session.getSessionId(),
				new ArrayBid.Builder(this.marketBasis).setDemand(0).build());
	...
	protected synchronized void publishNewPrice() {
		...
		if (this.objectiveEndpoint != null) {
			...
			Bid aggregatedObjectiveBid = this.objectiveEndpoint
					.handleAggregateBid(aggregatedBid);
			// aggregate again with device agent bid.
			Bid finalAggregatedBid = aggregatedBid
