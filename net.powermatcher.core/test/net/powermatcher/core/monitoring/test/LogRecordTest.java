...
public class LogRecordTest {
	...
	public void setUp() throws Exception {
		...
		incomingBidEvent = new IncomingBidEvent(clusterId, agentId, sessionId,
				timeStamp, fromAgentId, bid, qualifier);
		outgoingBidEvent = new OutgoingBidEvent(clusterId, agentId, sessionId,
				timeStamp, bid, qualifier);
		incomingPriceUpdateEvent = new IncomingPriceUpdateEvent(clusterId,
				agentId, sessionId, timeStamp, priceUpdate, qualifier);
		outgoingPriceUpdateEvent = new OutgoingPriceUpdateEvent(clusterId,
				agentId, sessionId, timeStamp, priceUpdate, qualifier);
	}
	...
	public void testBidLogRecordIncomingBid() {
		...
		BidLogRecord bidLogRecord = new BidLogRecord(incomingBidEvent, logTime,
				dateFormat);
		...
		assertThat((SimpleDateFormat) bidLogRecord.getDateFormat(),
				is(equalTo(dateFormat)));
	...
	public void testBidLogRecordOutgoingBid() {
		...
		c.set(2014, 12, 12);
		Date logTime = c.getTime();
		BidLogRecord bidLogRecord = new BidLogRecord(outgoingBidEvent, logTime,
				dateFormat);
		assertThat(bidLogRecord.getClusterId(), is(equalTo(clusterId)));
		assertThat(bidLogRecord.getAgentId(), is(equalTo(agentId)));
		assertThat(bidLogRecord.getEventTimestamp(), is(equalTo(timeStamp)));
	...
	public void testPriceLogRecordIncomingPrice() {
		...
		c.set(2014, 12, 12);
		Date logTime = c.getTime();
		PriceUpdateLogRecord priceUpdateLogRecord = new PriceUpdateLogRecord(
				incomingPriceUpdateEvent, logTime, dateFormat);
		...
		assertThat(priceUpdateLogRecord.getEventTimestamp(),
				is(equalTo(timeStamp)));
		assertThat((SimpleDateFormat) priceUpdateLogRecord.getDateFormat(),
				is(equalTo(dateFormat)));
		...
		assertThat(priceUpdateLogRecord.getPriceUpdate(),
				is(equalTo(priceUpdate)));
	}
	...
	public void testPriceLogRecordOutgoingPrice() {
		...
		Date logTime = c.getTime();
		PriceUpdateLogRecord priceUpdateLogRecord = new PriceUpdateLogRecord(
				outgoingPriceUpdateEvent, logTime, dateFormat);
		assertThat(priceUpdateLogRecord.getClusterId(), is(equalTo(clusterId)));
		assertThat(priceUpdateLogRecord.getAgentId(), is(equalTo(agentId)));
		assertThat(priceUpdateLogRecord.getEventTimestamp(),
				is(equalTo(timeStamp)));
