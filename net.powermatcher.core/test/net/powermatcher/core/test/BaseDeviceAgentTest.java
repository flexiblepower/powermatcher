...
public class BaseDeviceAgentTest {
	...
	public void testConnectToMatcher() {
		Session session = new SessionImpl(null, null, "testAgent", null,
				"testMatcherId", "testSession");
	...
	public void testCreateBidSessionNull() {
		PricePoint[] pricePoints = new PricePoint[] {
				new PricePoint(new Price(marketBasis, 3), 10.0),
	...
	public void testUpdatePrice() {
		...
		assertThat(observer.incomingPriceEvent.getPriceUpdate(),
				is(equalTo(priceUpdate)));
	}

	@Test
	public void testPublishBid() {
		ArrayBid bid = new ArrayBid(marketBasis, 0, new double[] { 5.0, 3.0,
				1.0, -1.0, -3.0 });
		...
		Session session = new SessionImpl(null, null, "testAgent", matcher,
				"mock", "testSession");
		baseDeviceAgent.connectToMatcher(session);
		baseDeviceAgent.publishBid(bid);
		assertThat((ArrayBid) observer.outgoingBidEvent.getBid(),
				is(equalTo(bid)));
	...
	public void testEquals() {
		...
		ArrayBid bid = new ArrayBid(marketBasis, 0, new double[] { 5.0, 3.0,
				1.0, -1.0, -3.0 });
		MockMatcherAgent matcher = new MockMatcherAgent("mock");
		Session session = new SessionImpl(null, baseDeviceAgent, "testAgent",
				matcher, "mock", "testSession");
		Session sessionOther = new SessionImpl(null, another, "testAgent",
				matcher, "mock", "testSession");
		baseDeviceAgent.connectToMatcher(session);
		baseDeviceAgent.publishBid(bid);
