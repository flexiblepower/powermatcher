...
public class ShowBidGraph extends AbstractHandler {
	@Override
	public Object execute(ExecutionEvent event) throws ExecutionException {
			...
			for (IEditorReference ref : references) {
				try {
					...
					if (editorInput instanceof FigureEditorInput && node.getId().equals(editorInput.getName())
							&& ((FigureEditorInput) editorInput).getGraphType() == GraphType.BID) {
						// ref.getEditor(true);
		...
		BidTimeSeriesXYGraph graph = new BidTimeSeriesXYGraph();
		graph.setTitle("Bid at " + node.getId());
		...
		FigureEditorInput input = new FigureEditorInput(node.getId(), new ToolbarArmedXYGraph(graph), GraphType.BID);
