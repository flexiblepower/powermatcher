...
public class CsvExpectedResultsReader {
	...
	private void readResultsData() throws IOException, DataFormatException {	
		...
		//this.deductMarketBasis(demandList);
		//this.marketBasis = createMarketBasis(marketBasisData);
		this.aggregatedBid = new BidInfo(marketBasis, createAggregatedBid(demandList));
	...
	private MarketBasis createMarketBasis(List<String> marketBasisData) throws DataFormatException {
		if (marketBasisData != null && marketBasisData.size() > 0) {
			...
			try {
				...
				MarketBasis mb = new MarketBasis(commodity, currency, priceSteps, minimumPrice, maximumPrice, significance, marketRef);
				this.marketBasis = mb;
			}
			catch (NumberFormatException e) {
