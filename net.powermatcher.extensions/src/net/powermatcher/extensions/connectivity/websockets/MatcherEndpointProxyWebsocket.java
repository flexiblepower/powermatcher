...
/**
 * WebSocket implementation of an {@link MatcherEndpointProxy}. Enabled two
 * agents to communicate via WebSockets and JSON over a TCP connection.
 * 
 * @author FAN
 * @version 2.0
 */
...
public class MatcherEndpointProxyWebsocket extends BaseMatcherEndpointProxy {
	...
	public synchronized void activate(Map<String, Object> properties) {
		...
		try {
			this.powermatcherUrl = new URI(config.powermatcherUrl()
					+ "?agentId=" + this.getAgentId() + "&desiredConnectionId="
	 ...
	 * {@inheritDoc}
	 * 
	 * This specific implementation serializes the {@link Bid} to json and sends
	 * it through the websocket.
	 */
	@Override
	...
	public void onMessage(String message) {
		...
		try {
			...
			if (pmMessage.getPayloadType() == PayloadType.CLUSTERINFO) {
				...
				this.updateRemoteMarketBasis(ModelMapper
	...
	private boolean connectWebsocket() {
		...
		try {
			...
			Future<org.eclipse.jetty.websocket.api.Session> connectFuture = client
					.connect(this, this.powermatcherUrl, request);
			...
			org.eclipse.jetty.websocket.api.Session newRemoteSession = connectFuture
					.get(this.connectTimeout, TimeUnit.SECONDS);
