...
public class PowermatcherWebSocket {
	private static final Logger LOGGER = LoggerFactory
			.getLogger(PowermatcherWebSocket.class);

	private static final Map<String, AgentEndpointProxyWebsocket> AGENT_ENDPOINT_PROXIES = Collections
			.synchronizedMap(new HashMap<String, AgentEndpointProxyWebsocket>());

	private static final Map<org.eclipse.jetty.websocket.api.Session, AgentEndpointProxyWebsocket> REMOTE_LOCAL_LINK = Collections
			.synchronizedMap(new HashMap<org.eclipse.jetty.websocket.api.Session, AgentEndpointProxyWebsocket>());
	...
	public synchronized void deactivate() {
		...
		for (Entry<org.eclipse.jetty.websocket.api.Session, AgentEndpointProxyWebsocket> link : REMOTE_LOCAL_LINK
				.entrySet()) {
			link.getKey().close();
		}
	}
	...
	public synchronized void onOpen(
			final org.eclipse.jetty.websocket.api.Session remoteSession) {
		Map<String, String> queryString = null;
		try {
			queryString = splitQuery(remoteSession.getUpgradeRequest()
					.getRequestURI());
		} catch (UnsupportedEncodingException e1) {
			remoteSession.close();
			LOGGER.warn(
					"Rejecting connection from remote agent [{}], URL is not complete (missing querystring)",
					this.remoteMatcherEndpointId);
			return;
		}

		// Read desired connection from the querystring
		this.desiredConnectionId = queryString.get("desiredConnectionId");
		if (this.desiredConnectionId == null
				|| this.desiredConnectionId.length() == 0) {
			remoteSession.close();
			LOGGER.warn(
					"Rejecting connection from remote agent [{}], desiredConnectionId is missing from querystring",
					this.remoteMatcherEndpointId);
			return;
		}
		...
		if (this.remoteMatcherEndpointId == null
				|| this.remoteMatcherEndpointId.length() == 0) {
			remoteSession.close();
			LOGGER.warn(
					"Rejecting connection from remote agent [{}], agentId is missing from querystring",
					this.remoteMatcherEndpointId);
			return;
		}
		...
		if (!AGENT_ENDPOINT_PROXIES.containsKey(desiredConnectionId)) {
			LOGGER.warn(
					"Rejecting connection from remote agent [{}] for non-existing local agent: [{}]",
					this.remoteMatcherEndpointId, this.desiredConnectionId);
		...
		AgentEndpointProxyWebsocket proxy = AGENT_ENDPOINT_PROXIES
				.get(desiredConnectionId);
		try {
			proxy.remoteAgentConnected(remoteSession);
		} catch (OperationNotSupportedException e) {
			LOGGER.warn(
					"Rejecting connection from remote agent [{}], reason: [{}]",
					this.remoteMatcherEndpointId, e);
			remoteSession.close();
		}
		...
		LOGGER.info("Remote agent [{}] connected to local agent [{}]",
				this.remoteMatcherEndpointId, this.desiredConnectionId);
	}
	...
	public synchronized void onClose(
		...
		AgentEndpointProxyWebsocket proxy = REMOTE_LOCAL_LINK
				.remove(remoteSession);
		LOGGER.info(
				"Agent disconnect detected remote agent: [{}], local agent",
	...
	public void onMessage(
			org.eclipse.jetty.websocket.api.Session remoteSession,
			String message) {
		// Find existing session
		if (!REMOTE_LOCAL_LINK.containsKey(remoteSession)) {
		...
		AgentEndpointProxyWebsocket proxy = REMOTE_LOCAL_LINK
				.get(remoteSession);
		LOGGER.info(
				"Received bid update from remote agent [{}] for local agent [{}]",
				proxy.getMatcherEndpointProxyId(), proxy.getAgentId());
