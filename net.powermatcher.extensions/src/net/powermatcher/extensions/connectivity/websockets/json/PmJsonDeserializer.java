...
/**
 * Implementation of JsonDeserializer for a PmMessage. Handles the correct
 * deserialization according to {@link PmMessage.PayloadType}
 * 
 * @author FAN
 * @version 2.0
 */
public class PmJsonDeserializer implements JsonDeserializer<PmMessage> {
	...
	public PmMessage deserialize(JsonElement json, Type desiredType,
		...
		if (message.getPayloadType() == PmMessage.PayloadType.PRICE_UPDATE) {
			message.setPayload((PriceUpdateModel) context.deserialize(payload,
					PriceUpdateModel.class));
		} else if (message.getPayloadType() == PmMessage.PayloadType.BID) {
			message.setPayload((BidModel) context.deserialize(payload,
					BidModel.class));
		} else if (message.getPayloadType() == PmMessage.PayloadType.CLUSTERINFO) {
			message.setPayload((ClusterInfoModel) context.deserialize(payload,
					ClusterInfoModel.class));
		}
