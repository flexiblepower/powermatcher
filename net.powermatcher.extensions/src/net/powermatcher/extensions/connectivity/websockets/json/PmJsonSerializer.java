...
/**
 * Implements a serializer for the net.powermatcher.api.data data types to JSON.
 * Items are wrapped in a {@link PmMessage} and correct type is set.
 * 
 * @author FAN
 * @version 2.0
 */
public class PmJsonSerializer {
	...
	public String serializeBid(final Bid bid) {
		...
		bidModel.setMarketBasis(ModelMapper.convertMarketBasis(bid
				.getMarketBasis()));
		...
		if (bid instanceof ArrayBid) {
			...
			bidModel.setPricePoints(ModelMapper
					.convertPricePoints(((PointBid) bid).getPricePoints()));
		}
	 ...
	 * @param bid
	 *            the bid to serializer
	 * @return a JSON string with a {@link PriceUpdate} wrapped in
	 *         {@link PmMessage}.
	 */
	public String serializePriceUpdate(final PriceUpdate priceUpdate) {
		...
		priceModel.setMarketBasis(ModelMapper.convertMarketBasis(priceUpdate
				.getPrice().getMarketBasis()));
	 ...
	 * @return a JSON string with clusterId and {@link MarketBasis} wrapped in
	 *         {@link PmMessage}.
	 */
	public String serializeClusterInfo(final String clusterId,
			final MarketBasis marketBasis) {
		ClusterInfoModel customerModel = new ClusterInfoModel();
		customerModel.setClusterId(clusterId);
		customerModel.setMarketBasis(ModelMapper
				.convertMarketBasis(marketBasis));
	...
	public PmMessage deserialize(String message) {
		Gson gson = new GsonBuilder().registerTypeAdapter(PmMessage.class,
				new PmJsonDeserializer()).create();
