...
/**
 * Helper class to mapp between net.powermatcher.api.data classes and model
 * classed for wire transfer.
 * 
 * @author FAN
 * @version 2.0
 */
public class ModelMapper {
	...
	public static Bid mapBid(BidModel bidModel) {
		...
		if (pricePointsModel == null || pricePointsModel.length == 0) {
			bid = new ArrayBid(marketBasis, bidModel.getBidNumber(),
					bidModel.getDemand());
		} else {
			bid = new PointBid(marketBasis, bidModel.getBidNumber(),
	...
	public static PriceUpdate mapPriceUpdate(PriceUpdateModel priceUpdateModel) {
		Price price = new Price(
				convertMarketBasis(priceUpdateModel.getMarketBasis()),
				priceUpdateModel.getPriceValue());
		PriceUpdate priceUpdate = new PriceUpdate(price,
				priceUpdateModel.getBidNumber());
		return priceUpdate;
	}
	 ...
	 */
	public static PricePoint[] convertPricePoints(MarketBasis marketBasis,
			PricePointModel[] pricePointsModel) {
		// Convert price points
		PricePoint[] pricePoints = new PricePoint[pricePointsModel.length];
		for (int i = 0; i < pricePoints.length; i++) {
			pricePoints[i] = new PricePoint(marketBasis,
					pricePointsModel[i].getPrice(),
	...
	public static PricePointModel[] convertPricePoints(PricePoint[] pricePoints) {
		...
		for (int i = 0; i < pricePoints.length; i++) {
			...
			pricePointsModel[i].setPrice(pricePoints[i].getPrice()
					.getPriceValue());
		}
	 ...
	 */
	public static MarketBasis convertMarketBasis(
			MarketBasisModel marketBasisModel) {
